...
/**
 * 컬렉션별 필드 정의 (누락 필드 추가용 — 추가만 하고 기존 필드는 건드리지 않음).
 * 앱 클라이언트 provisioning.ts 의 정의와 동일하게 유지한다.
 */
const BZ_SCHEMA_FIELDS = {
  users: [
    { name: "role", type: "text" },
    { name: "battlezone_points", type: "number" },
    { name: "nickname_color", type: "text" },
    { name: "profile_avatar", type: "text" },
    { name: "owned_items", type: "text" },
    { name: "pubg_nickname", type: "text" },
  ],
  notices: [
  ...
  matches: [
    ...
    { name: "date", type: "text" },
    { name: "map", type: "text", required: true },
    { name: "mode", type: "text" },
    { name: "result", type: "text" },
    { name: "score_us", type: "number" },
    { name: "score_them", type: "number" },
    { name: "kills", type: "number" },
    { name: "deaths", type: "number" },
    { name: "assists", type: "number" },
    { name: "headshots", type: "number" },
  ...
  rankings: [
    ...
    { name: "streak", type: "text" },
  ],
  game_settings: [
    { name: "target_kills", type: "number" },
    { name: "season", type: "text" },
    { name: "elo_k", type: "number" },
    { name: "initial_elo", type: "number" },
    { name: "match_elo_range", type: "number" },
    { name: "matching_enabled", type: "bool" },
    { name: "game_start_timeout_min", type: "number" },
  ...
  kill_queue: [
    ...
    { name: "battle_id", type: "text" },
    { name: "season", type: "text" },
  ],
  kill_battles: [
    { name: "season", type: "text" },
    { name: "target_kills", type: "number" },
    { name: "player_a", type: "text", required: true },
    { name: "player_b", type: "text", required: true },
    { name: "kills_a", type: "number" },
    { name: "kills_b", type: "number" },
    { name: "pending_kills_a", type: "number" },
    { name: "pending_kills_b", type: "number" },
    { name: "status", type: "text" },
    { name: "winner", type: "text" },
    { name: "winner_pending", type: "text" },
    { name: "started_a", type: "bool" },
    { name: "started_b", type: "bool" },
    { name: "playing_at", type: "text" },
    { name: "game_started_a", type: "bool" },
    { name: "game_started_b", type: "bool" },
    { name: "current_round_a", type: "number" },
    { name: "current_round_b", type: "number" },
    { name: "finished_at", type: "text" },
    { name: "elo_delta_a", type: "number" },
    { name: "elo_delta_b", type: "number" },
  ],
  kill_rounds: [
    { name: "battle", type: "text", required: true },
    { name: "player", type: "text", required: true },
    { name: "round_number", type: "number", required: true },
    { name: "status", type: "text" },
    { name: "kills_manual", type: "number" },
    { name: "kills_api", type: "number" },
    { name: "kills_final", type: "number" },
    { name: "match_id", type: "text" },
    { name: "game_started_at", type: "text" },
    { name: "map", type: "text" },
    { name: "placement", type: "number" },
    { name: "match_ended_at", type: "text" },
    { name: "verified_at", type: "text" },
    { name: "note", type: "text" },
  ],
  kill_logs: [
    { name: "battle", type: "text" },
...
/** 누락된 컬렉션 필드를 추가한다 (기존 필드/데이터는 변경하지 않음). */
function BZSyncSchema() {
  const results = [];
  for (const [name, fields] of Object.entries(BZ_SCHEMA_FIELDS)) {
    let col = null;
    try {
      col = $app.findCollectionByNameOrId(name);
    } catch (e) {
      results.push({ name, status: "컬렉션 없음" });
      continue;
    }
    let added = 0;
    for (const f of fields) {
      if (col.getFieldByName(f.name)) continue;
      const nf = new SchemaField();
      nf.name = f.name;
      nf.type = f.type;
    ...
    if (added > 0) {
      try {
        ...
        results.push({ name, status: "저장 실패: " + String((e && e.message) || e) });
        continue;
      }
    }
    results.push({ name, status: added > 0 ? added + "개 필드 추가" : "정상" });
  }
  return results;
}
